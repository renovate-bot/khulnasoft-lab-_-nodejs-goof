...
User.find({ username: 'admin@khulnasoft.com' }).exec(function (err, users) {
  console.log(users);
  if (users.length === 0) {
    console.log('no admin');
    new User({ username: 'admin@khulnasoft.com', password: 'SuperSecretPassword' }).save(function (err, user, count) {
      if (err) {
